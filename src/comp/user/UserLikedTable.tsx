...
const UserLikedTable: React.FC<IProps> = (props) => {
  ...
  const handleLikedData = useCallback(async (date: string) => {
    if (!Object.keys(userDfsWeb3Datas).length || !vip) {//判断对象是否为空
      setDataSource([]);
      return;
    }

    let likedAccounts: {acc: string, time: string, id: number, type: string, trx: string}[] = [];
    date = moment(date).diff('2022-10-30', 'days').toString();
    let accountId = accountMapId[props.account];
    if (userDfsWeb3Datas.hasOwnProperty(date)) { //过了24点后，如果没有数据，props.actionData[date]为undefined，props.actionData[date].account_map会报错，所以这里加undefined判断
      ...
      if (accountMap.hasOwnProperty(accountId)) {
        likedAccounts = accountMap[accountId].likedAcc as {acc: string, time: string, id: number, type: string, trx: string}[];
      }
    }
    ...
    if (props.accountFlag === '1') {
      dispatch(actions.setYouLikedCount(index-1));
    } else if (props.accountFlag === '2') {
      dispatch(actions.setHeLikedCount(index-1));
    }
    setDataSource(dataArray);
  }, [props, handleShowName, vip, profiles, userDfsWeb3Datas, dfsPrice, priceOption, dispatch, idMapAccount, accountMapId]);

  const handleDateChange = useCallback((date: any, dateString: any) => {
    setDefaultValue(date);
    handleLikedData(dateString);
  },[handleLikedData]);

  useEffect(() => {
    handleLikedData(defaultValue.format('YYYY-MM-DD'));
  },[handleLikedData, defaultValue]);
...
export default memo(UserLikedTable);
