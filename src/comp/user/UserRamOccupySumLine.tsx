...
const UserRamOccupySumLine: React.FC<IActionData> = (props) => {
  ...
  let ramValueSum: number = 0;
  let eosValueSum: number = 0;
  let day: string;
  ...
  for (const key in userDfsWeb3Datas) {
    ...
    if ((userDfsWeb3Datas[key] as UserDfsWeb3DataType).hasOwnProperty(accountId)) {
      ...
      if (flag) {
        let ramValue: number = ram / 1024;
        let eosValue: number = ramValue * ramPrice;
        ramValueSum = Number((ramValueSum + ramValue).toFixed(1));
        eosValueSum = Number((eosValueSum + eosValue).toFixed(2));
        data.push({day, value: ramValueSum, category: '内存消耗(KB)'});
        data.push({day, value: eosValueSum, category: `内存EOS估值(${ramPrice.toPrecision(2)}EOS/KB))`});
      }
    } else {
      if (flag) {
        data.push({day, value: ramValueSum, category: '内存消耗(KB)'});
...
export default memo(UserRamOccupySumLine);