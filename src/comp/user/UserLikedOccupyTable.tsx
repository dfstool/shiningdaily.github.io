...
const UserLikeOccupyTable: React.FC<IProps> = (props) => {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    {
      title: <span className="fw-bold">读者</span>,
      dataIndex: 'user',
      key: 'user',
      align: 'center',
      render: (text: string, record, index) => {
  ...
  const handleLikeData = useCallback(async () => {
    ...
    type LikedAccountsType = {acc: string, time: string, id: number, type: string, trx: string}[];
    let accountsData: {[index: string]: {liked: number, reward: number}} = {};
    let rewardSum: number = 0;
    let likedSum: number = 0;
    let accSet: string[] = [];
    ...
    for (let date in userDfsWeb3Datas) {
      let likedAccounts: LikedAccountsType = [];
      const accountMap = userDfsWeb3Datas[date] as UserDfsWeb3DataType;
      if (accountMap.hasOwnProperty(accountId)) {
        likedAccounts = accountMap[accountId].likedAcc as LikedAccountsType;
      } else {
        continue;
      } 
      for (let item of likedAccounts) {
        ...
        const actions = (await db.shiningpool1.where('trx_id').startsWith(trx_id).toArray()).filter((value) => {
          if (value.account === "minedfstoken" && value.name === "transfer" && value.data.from === "shiningpool1" && 
              value.data.memo === "reader content mining reward" && value.data.to === idAccount && value.receiver === idAccount) {
                return true;
              } else {
                return false;
              }
        ...
        if (accountsData.hasOwnProperty(idAccount)) {
          accountsData[idAccount].liked += 1;
          accountsData[idAccount].reward += reward;
        } else {
          accountsData[idAccount] = {liked: 1, reward};    
        }
    ...
    if (rewardSum > 0.0001) {
      for (let account in accountsData) {
        ...
        dataArray.push({ key: 0, index: 0, user: handleShowName(nick, account), like: accountsData[account].liked,
      ...
      for (let account in accountsData) {
        ...
        dataArray.push({ key: 0, index: 0, user: handleShowName(nick, account), like: accountsData[account].liked,
          reward: Number((accountsData[account].reward * dfsPrice[priceOption]).toFixed(4)), account,
          percent: (accountsData[account].liked / likedSum * 100).toFixed(1) + '%'});
      }
    ...
    if (props.accountFlag === '1') {
      dispatch(actions.setYouLikedAccountCount(accSet.length));
    } else if (props.accountFlag === '2') {
      dispatch(actions.setHeLikedAccountCount(accSet.length));
    }
