...
const UserActionsLine: React.FC<IProps> = (props) => {
  const data: any[] = [];
  ...
  for (const key in shiningPoolDatas) {
    ...
    if ((shiningPoolDatas[key] as ShiningPoolDataType).author.hasOwnProperty(accountId)) {
      ...
      const reward = (shiningPoolDatas[key] as ShiningPoolDataType).author[accountId].reward as number;
      if (like) { //找出首个like为非0的开始时间
        flag = true;
      }
      if (flag) {
        data.push({day, value: like, category: '有效获赞数'});
        data.push({day, value: Number((reward * dfsPrice[priceOption]).toFixed(4)), category: `奖励${priceOption}数`});
      }
    } else {
      if (flag) {
        data.push({day, value: 0, category: '有效获赞数'});
        data.push({day, value:0.000, category: `奖励${priceOption}数`});
      }      
    }
  ...
  for (const key in userDfsWeb3Datas) {
    ...
    if ((userDfsWeb3Datas[key] as UserDfsWeb3DataType).hasOwnProperty(accountId)) {
      ...
      if (flag) {
        data.push({day, value: newpost, category: '作品数'});
      }
    } else {
      if (flag) {
        data.push({day, value: 0, category: '作品数'});
      }      
    }
  }
...
export default memo(UserActionsLine);
